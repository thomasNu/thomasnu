 ...
 * </output>
 */
class Tx_Thomasnu_ViewHelpers_TranslateViewHelper extends Tx_Fluid_Core_ViewHelper_AbstractViewHelper {
	...
	public function render($key, $default = NULL, $htmlEscape = TRUE, array $arguments = NULL) {
		...
		$parts = explode('.', $key, 2);
			switch ($parts[0]) {
				case 'common':
					$key = 'LLL:EXT:lang/locallang_common.xml:' . $parts[1];
					if ($parts[1] == 'to') $mod = 'strtolower';
					break;
			}
		...
		if ($value === NULL) {
			...
			if ($value == '') $value = '[' . $parts[0] . '.' . $parts[1] . ']';  // tnu:
		} elseif ($htmlEscape) {
			$value = htmlspecialchars($value);
		}
		if ($mod == 'strtolower') $value = strtolower($value);
